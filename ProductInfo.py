...
kHdmiVersion = "hdmiver"
kPhonoType = "phonotype"
kDisplayType = "displaytype"
kVarsKey = "variants"
kPcasInfoKeys = [kDescKey, kVerKey, kTargetKey, kNameKey, kPlatKey, kRenewNameKey, kRenewUrlKey, kUrlKey, kVolkano1Key, kVolkano1SourcesKey, kSourceMapRcaFirst, kHdmiVersion, kPhonoType, kDisplayType]
...
"""
    ...
    cpp += "    static const TChar* kKeyPcasHdmiVersion = \"%s\";\n" % kHdmiVersion
    cpp += "    static const TChar* kKeyPcasPhonoType = \"%s\";\n" % kPhonoType
    cpp += "    static const TChar* kKeyPcasDisplayType = \"%s\";\n" % kDisplayType
...
"""
    for pcas, pcasinfo in jsonObjs[kPcasTableName].iteritems():
        ...
        for var in varList:
            ...
            try:
                ...
                pass # peripheral board (no firmware variant)
            cpp += "} },\n"
    ...
    public:
        ...
        static TUint GetHdmiVersion(TUint aPcas)
        {
            try {
                return GetNum(kKeyPcasHdmiVersion, aPcas);
            }
            catch(...) { }
            return 0;
        }

        static const TChar* GetPhonoType(TUint aPcas)
        {
            try {
                return Get(kKeyPcasPhonoType, aPcas);
            }
            catch(...) { }
            return "";
        }

        static const TChar* GetDisplayType(TUint aPcas)
        {
            try {
                return Get(kKeyPcasDisplayType, aPcas);
            }
            catch(...) { }
            return "";
        }

        static TBool IsMovingMagnet(TUint aPcas)
        {
            return  strcmp(GetPhonoType(aPcas), "mm") == 0;
        }

        static TBool IsMovingCoil(TUint aPcas)
        {
            return  strcmp(GetPhonoType(aPcas), "mc") == 0;
        }

        static TBool IsAkurateDisplay(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "vfd32large") == 0;
        }

        static TBool IsKlimaxDisplay(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "vfd32small") == 0;
        }

        static TBool IsKikoDisplay(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "oled64") == 0;
        }

        static TBool IsSelektDisplay(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "oled1080") == 0;
        }

        static TBool IsLed1Display(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "led1") == 0;
        }

        static TBool IsLed6Display(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "led6") == 0;
        }

        static TBool IsLed7Display(TUint aPcas)
        {
            return  strcmp(GetDisplayType(aPcas), "led7") == 0;
        }
...
def CreateRosXmlEntries( aFilePath ):
